//
//  fhooderNine.swift
//  Fhood
//
//  Created by Young-hu Kim on 7/18/15.
//  Copyright (c) 2015 Fhood LLC. All rights reserved.
//

import UIKit

func fhooderNine() {
    variables.name = "House of Shrimp"
    variables.ratingInString = "4andHalf-Spoon"
    variables.reviews = 127
    variables.foodType = ["Sandwich", "Shrimp Sandwich", "Lemonade"]
    variables.address = "47 Montclair Terrace, San Francisco, CA 94109"
    variables.fhooderLatitude = 37.802763
    variables.fhooderLongitude = -122.418868
    variables.distance = 1.3
    variables.pickup = true
    variables.eatin = false
    variables.delivery = false
    variables.phoneNum = "(415) 165-9135"
    variables.isOpen = false
    variables.isClosed = true
    variables.timeOpenHour = 2
    variables.timeOpenMinute = 0
    variables.timeOpenAmpm = "PM"
    variables.timeCloseHour = 2
    variables.timeCloseMinute = 0
    variables.timeCloseAmpm = "AM"

    variables.itemNames = ["Shrimp Sandwich", "Strawberry Lemonade"]
    variables.itemPrices = [5.0, 2.0]
    variables.itemCount = [0, 0]
    variables.itemIngredients = ["Shrimp, Bread, Cayenne Pepper, Mayonnaise, Lettuce.", "Lemon, Water, Strawberry, Sugar."]

    variables.fhooderFace = "fhooder9face"
    variables.fhooderFirstName = "Ike"

    variables.fhooderPic = "fhooder9"
}
